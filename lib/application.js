...
function get_mongo_uri(dbconfig) {
  ...
  function or(prop, def) {
    return (dbconfig && prop in dbconfig) ? dbconfig[prop] : def;
  }
  dbconfig.hostname = or('hostname', 'localhost'); 
  dbconfig.port = or('port', false );
  dbconfig.db = or('db', 'test');
  ...
  if(dbconfig.username && dbconfig.password) {
    ...
    return "mongodb://" + dbconfig.hostname + 
...
app.register = function(model) {
  ...
  if ('parent' in model) {
    var parentName = model.parent.toLowerCase();
    if (parentName in this.models) {
      model.parent = this.models[parentName];
    } else {
      throw "No registered model by name " + parentName + ".\nMake sure you register the parent before " + name + " and spelled everything correctly";
    }
  } else {
    model.parent = null
  }
