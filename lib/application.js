...
app.init = function(dbconfig) {
  ...
  var self = this;
  MongoClient.connect(get_mongo_uri(dbconfig), function(err, db) {
    console.log(err); 
    self.db = db;
  });
}
...
app.register = function(model) {
  ...
  if (model.hasOwnProperty('parent')) {
    ...
    if (this.models.hasOwnProperty(parent)) {
      model.parent = this.modelMap[model.parent.toLowerCase()];
    } else {
  ...
  console.log("Registering fn at " + model.title.toLowerCase());
  this.use('/' + model.title.toLowerCase(), function(req, res, next) {
    console.log(req);
    res.end("Called on " + model.title.toLowerCase());

  });
  //this._router.register(model, this.db);
}
