...
Model.filter = function(req, quer) {
  ...
  // filter by fields
  [req.body, req.query].forEach(function(filterableData) {
    Object.keys(filterableData).forEach(function(filterField) {
      var field = filterField.split('__');
      var filter_func = field[1] || 'equals';
      if(field[0] in quer.model.schema.paths && filter_func in valid_filters_funcs) {
        var data = filterableData[filterField];
        if(data === 'true')
          data = true;
        else if(data === 'false')
          data = false;
        quer = valid_filters_funcs[filter_func](data, quer.where(field[0]));
      }
    });
  });
