...
Model.prototype.register = function(app, url) {
  var self = this;
  //app.all(url + '*', function() { self.preprocess.apply(self, arguments); });
 ...
 * and in req.body and req.query, the querystring.
 * 
 * Returns the filtered query 
 *
 */
/*Model.prototype.filter = function(filters, body, querystring, quer) {
  filters.forEach(function(filter) {
    if ('val' in filter) {
      quer = quer.where(filter.key, filter.val);
    }
  });
  for (var key1 in body) {
    if (key1 in filters) {
      quer = filters[key1](body[key1], query);
    }
  }
  for (var key2 in querystring) {
    if (key2 in valid_filters) {
      quer = valid_filters[key2](querystring[key2], quer);
    }
  }
  return quer;
}
