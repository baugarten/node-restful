...
Model.prototype.addDefaultRoutes = function() {
  ...
  methods.forEach(function(meth) {
    if (meth in self.methods) {
      ...
      self.registerRoute(meth, self.methods[meth]);
    }
  });

  defaultroutes.forEach(function(route) {
    self.registerRoute('__all__', route, handlers[route].call(self));
  });
}
  ...
  this.schema.eachPath(function(pathName, schemaType) {
    if (schemaType.options.type === 'ObjectId') {
      ...
      ['get'].forEach(function(method) {
        self.registerRoute(method, pathName, handlers[method + 'Path'].call(self, pathName));
      }); 
    }
  });
}
...
Model.prototype.addUserRoutes = function() {
  for (var route in this.userroutes) {
    this.userroute(route, this.userroutes[route]);
  };
}

Model.prototype.route = function(path, detail, fn) {
  path = path.replace(/\//g, /\./);
  ...
  subPaths.forEach(function(sub, i) {
    if (i === subPaths.length && arguments.length >= 2) {
      route[sub] = handlers.wrap.call(this, path, detail, fn);
    }
...
Model.prototype.userroute = function(route, fn) {
  ...
  if (!('function' === typeof fn)) {
    detail = !!fn.detail;
    fn = fn.handler;
    methods = fn.methods || ['__all__'];
  }
  methods.forEach(function(meth) {
    self.registerRoute(meth, route, detail, fn);
  });
}

Model.prototype.registerRoute = function(meth, path, detail, fn) {
  ...
  route[meth] = handlers.wrap.call(this, path, detail, fn);
}
