...
Model.prototype.dispatch = function(req, res, next) {
  ...
  if (method in handler) return handler = handler[method];
...
Model.prototype.getPath = function(pathName) {
  var self = this;
  return function(req, res, next) {
    req.quer.populate(pathName).findOne();
    self.requireOneModel(res, req.quer, function() {
...
Model.prototype.put = function() {
  ...
  return function(req, res, next, filters) {
    ...
    self.requireOneModel(res, req.quer, function(err, newObj) {
      if (err) {
...
Model.prototype.delete = function() {
  ...
  return function(req, res, next, params) {
    ...
    self.requireOneModel(res, req.quer, function(err, obj) {
 ...
 *
 */
Model.prototype.requireOneModel = function(res, quer, callback) {
  // Make the haveOneModel more robust by checking all UNIQUE fields
  var haveOneModel = ('id' in quer._conditions); 

  if (!haveOneModel) {
    this.Obj.find(quer._conditions).limit(2).exec(function(err, list) {
      if (list.length > 1) {
        respond(res, 404, {
