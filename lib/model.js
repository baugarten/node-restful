...
Model.prototype.addSchemaRoutes = function() {
  ...
  this.schema.eachPath(function(pathName, schemaType) {
    ...
    if (parts.length > 1) {
      ...
      schemaType.path = pathName;
      self.addResource(childObj, pathName, schemaType);
    }
  });
}

Model.prototype.addResource = function(objName, pathName, schemaType) {
  var child = (objName in this.children) ? this.children[objName] : {
    title: objName,
    methods: this.methods,
    parent: this.resourceName,
    schema: mongoose.Schema({
    }),
  };
  child.schema.path(pathName, schemaType.options);
  this.children[objName] = child;
}
...
Model.prototype.register = function(app) {
  ...
  console.log(this);
  var self = this;
  self.registerSchemaRoutes(app);
    console.log("Registering routes at " + this.baseurl);
  app.use(this.baseurl, function() { self.dispatch.apply(self, arguments); });
}

Model.prototype.registerSchemaRoutes = function(app) {
  console.log("Register schema routes");
  console.log(this.children);
  for (var objName in this.children) {
    this.children[objName] = app.register(this.children[objName]);
  }
}
...
Model.prototype.dispatch = function(req, res, next) {
  ...
  } else if (route in this.children) {
    console.log("ROUTE IS IN MY CHILDREN");
    console.log(this.children[route]);
    return this.children[route].dispatch(req, res, next);
  }
  ...
  return next(); 
  return (nextModel) ? next() : respond(res, 404, {
...
Model.prototype.parseUrl = function(url) {
  ...
  next = urlparts.length > ((filters !== '') ? 1 : 0);
  if (filters !== '') urlparts[0] = '';
  if (next) urlparts.splice(0, 1); urlparts[0] = '';
  return {
    filters: filters,
    route: route, 
    next: next,
...
Model.prototype.get = function() {
  ...
  return function(req, res, next, params) {
    ...
    req.queryset.lean().exec(function(err, list) {
      if (err)
