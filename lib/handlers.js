...
exports.get = function(req, res, next) {
  ...
  // count model rows asynchronously
  this.count({}, function(err, count){
    
    // return on error; NOT SURE WHETHER THIS WORKS
    if (err) {
    ...
    // initial values for start and end row
    var row_start=0;
	  var row_end=10000;
    ...
    if(range !== null && range !== undefined){
      ...
      row_start=parseInt(m[0]);
      row_end=parseInt(m[1]);
    }
    
    // set range parameter for model query
    req.quer.skip(row_start).limit(row_end-row_start+1);
    
    // set sort parameter for model query
    var req_url=req.originalUrl;
    
    if (req_url.indexOf('sort(')>-1){
      var sort_par=req_url.slice(req_url.indexOf('(')+1,req_url.indexOf(')'));
      req.quer.sort(JSON.parse('{"'+sort_par.slice(1)+'":'+((sort_par.slice(0,1) === '-') ? -1 : 1)+'}'));
    }
  
    // set response parameter for content range
    res.header('Content-Range',row_start+'-'+row_end+'/'+row_count)
