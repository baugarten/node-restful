...
exports.put = function(req, res, next) {
    ...
    if (this.shouldUseAtomicUpdate) {
      req.quer.findOneAndUpdate({}, req.body, {overwrite: true, runValidators: true, new: true}, function(err, newObj) {
        if (err) {
          exports.respond(res, 500, err);
        } else if (!newObj) {
          exports.respond(res, 404, exports.objectNotFound());
      ...
      req.quer.findOne({"_id": req.params.id}, function(err, docToUpdate) {
        ...
        docToUpdate.save(function (err, newObj) {
          exports.respondOrErr(res, 400, err, 200, newObj);
          next();
        });
      });
    }
...
exports.patch = function(req, res, next) {
  ...
  // Update in 1 atomic operation on the database, PATCH must be atomic
  this.update_options['new'] = true;
  req.quer.findOneAndUpdate({"_id": req.params.id}, req.body, this.update_options, function(err, saved) {
    if (err) {
      exports.respond(res, 500, err);
    } else if (!saved) {
      exports.respond(res, 404, exports.objectNotFound());
