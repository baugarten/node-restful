exports.wrap = function(fn) {
  ...
  if ('function' !== typeof fn) {
    if ('function' === typeof fn.after) fn.after = [fn.after];
    if (fn.after) stack.push.apply(stack, fn.after);

    if (fn.detail) fn.handler = exports.detailWrap.call(this, fn.handler);
    stack.push(fn.handler);

    if ('function' === typeof fn.before) fn.before = [fn.before];
    if (fn.before) stack.push.apply(stack, fn.before);
  } else {
    stack.push(fn);
  }
...
exports.get = function(req, res, next) {
  req.quer.lean().exec(function(err, list) {
    exports.respondOrErr(res, 500, err, 200, (('_id' in req.quer._conditions) ? list[0] : list));
    next();
  });
}
...
exports.authFailure = function() {
  return {
    ...
    name: "Unauthorized",
    errors: 'Operation not authorzed on ' + this.resourceName,
  }
}
