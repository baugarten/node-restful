  ...
  if (req.params.id && !mongoose.Types.ObjectId.isValid(req.params.id)) {
    ...
    req.quer.exec(function(err, list) {
      if (err) {
        ...
        req.quer.countDocuments(function(err, total) {
          if (err) {
            exports.respond(res, 500, err);
          } else {
...
exports.put = function(req, res, next) {
  ...
  // Update in 1 atomic operation on the database if not specified otherwise
    req.quer.findOneAndReplace({'_id': req.params.id}, req.body, { omitUndefined: true, ...this.update_options}, function(err, newObj) {
      if (err) {
        exports.respond(res, 500, err);
      } else if (!newObj) {
        exports.respond(res, 404, exports.objectNotFound());
